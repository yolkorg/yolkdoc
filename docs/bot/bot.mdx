...
### Intents
Intents are a way to enable specific features that require extra API calls or use more processing power. The current intents are as follows:
...
Account credentials are returned and added to `bot.account`.
...
This is the current best method to log in using Google.

To obtain a refresh token from Shell, run the following console script:
...
This allows you to cleanly (and consistently) login with Google.

Account credentials are returned and added to `bot.account`.

### [async] bot.loginAnonymously
Logs you into a guest/anonymous account. This is the default behavior if you do not call either of the above authorization functions.
...
Modes can be found using the [GameMode constant](../classes/constants).

Maps can be found using the [Map constant](../classes/constants). It's just the map name.
...
if (bot.canSee(bot.players[0])) {
    console.log('I can see the first player!');
}
```
...
### bot.getBestTarget
Returns the closest player that is in the bot's LoS. Returns nothing if no one is in the bot's LoS.
...
- `bot.players[i].streakRewards` - the rewards the player has for their streak - a list of the [StreakRewards constant](../classes/constants)
...
- `bot.game.gameMode` - the game mode of the game (instanceof [GameMode](../classes/constants))
- `bot.game.map` - the map of the game (instanceof [Map](../classes/constants))
...
- `bot.game.options` - the options of the game
    ...
    - `bot.game.options.weaponsDisabled` - the weapons disabled in the game - in the order of [GunList](../classes/constants) - 1 = disabled, 0 = enabled
...
- `bot.game.stage` - the stage of the game (instanceof [CoopStates](../classes/constants))
