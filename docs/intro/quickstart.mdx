...
You can also choose to use the alpha version, where we test features still in development. This is not recommended for production use, and should be used if you have a specific feature you want to use. You can select the alpha version by replacing `yolkbot` with `yolkbot-alpha`.

<Note>We only officially use two package names: `yolkbot` & `yolkbot-alpha`. Do not install any others, as they are not supported and may lead to you being hacked.</Note>
