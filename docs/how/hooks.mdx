...
You can compare this to the `CloseCode` [constant](../classes/constants).

### collectAmmo
This is called when a player collects an ammo pack.

- `player` is a Player object
- `weapon` is a Gun class (see [Gun](../classes/constants))
...
- `player` is a Player object
- `streak` is a value of the [`ShellStreaks`](../classes/constants) enum
...
- `player` is a Player object
- `oldTeam` is a number (see the [`Teams`](../classes/constants) enum)
- `newTeam` is a number (see the [`Teams`](../classes/constants) enum)
