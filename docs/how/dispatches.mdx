...
## Dispatch Checks
Dispatches will always run a check to see if they can occur. This check just returns `true` or `false`, and you *should* do it on your own to make sure the Dispatch will succeed.
...
if (spawnDispatch.check()) {
    bot.dispatch(spawnDispatch);
}
```

If the check is false, the Dispatch will not run.
